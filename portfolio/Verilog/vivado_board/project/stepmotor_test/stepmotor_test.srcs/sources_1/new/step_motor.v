...
module step_motor(
    ...
    reg [1:0] state;                        // FSM(Finite State Mashine) 상태 레지스터
    localparam IDLE = 2'b00;                // 대기 상태
    localparam ROTATE_90_CW = 2'b01;        //90도 정회전
    localparam ROTATE_180_CCW = 2'b10;      //180도 역회전
    ...
    always @(posedge clk, posedge rst) begin
        if(rst) begin
            ...
            if(clk_div_cnt == 30'd500_000) begin    // ==> pre_scale
    ...
    always @(posedge clk_1Hz, posedge rst) begin
        if(rst) begin
            ...
            case(state)
                IDLE : begin
                    motor_data <= 4'b0000;
                    step_count <= 12'd0;    // 초기화
                    if(start_op) begin      // start_op 들어오면 90도 정회전 시작
                        target_steps <= STEPS_90_DEGREE;    state <= ROTATE_90_CW;
                    end
                end
                ...
                ROTATE_90_CW : begin
                    if(step_count < target_steps) begin
                        ...
                        target_steps <= STEPS_180_DEGREE;   state <= ROTATE_180_CCW;
                    end
                end
