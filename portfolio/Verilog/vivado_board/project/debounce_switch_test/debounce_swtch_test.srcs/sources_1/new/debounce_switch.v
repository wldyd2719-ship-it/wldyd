...
module debounce_switch(
    ...
    always @(posedge i_clk ) begin
        if ( ((i_switch == PUSHED) && (r_prevState == RELEASED)) && (r_count < DEBOUNCE_LIMIT) ) begin
                r_count <= r_count+1;            retValue <= FALSE;        
            end
        else if ( ((i_switch == PUSHED) && (r_prevState == RELEASED)) && (r_count == DEBOUNCE_LIMIT) ) begin
                r_count <= 0;  r_prevState <= PUSHED;      retValue <= FALSE;
            end 
        else if ( ((i_switch == PUSHED) && (r_prevState == PUSHED)) && (r_count < DEBOUNCE_LIMIT) ) begin            
                r_count <= r_count+1;            retValue <= FALSE;
            end
        else if ( ((i_switch == RELEASED) && (r_prevState == PUSHED)) && (r_count == DEBOUNCE_LIMIT) ) begin
                r_count <= 0;   r_prevState <= RELEASED;   retValue<=TRUE;
            end
//          else if ( ((i_switch == PUSHED) && (r_prevState == RELEASED)) && (r_count == DEBOUNCE_LIMIT) ) begin
//                r_count <= 0;  r_prevState <= RELEASED;      retValue <= TRUE;
//            end   
        else begin
                r_count <= 0;   retValue <= FALSE;
            end
    end
