...
def calc_angle(pt, org):
    dxy = np.subtract(pt, org).astype(float)        # 두 좌표간 차분 계산
    return cv2.fastAtan2(dxy[1], dxy[0])  # 차분으로 각도 계산
...
def onMouse(event, x, y, flags, param):
    ...
    if len(pts) == 3:
        angle1 = calc_angle(pts[1], pts[0])  # 회전각 계산
        angle2 = calc_angle(pts[2], pts[0])  # 회전각 계산
        angle = angle2 - angle1
